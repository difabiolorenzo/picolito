// Je ne comprends plus mon code... hich
...
function nextSentence() {
    if (game.cycle_state < game.sentence_amount) {
        ...
        if (game.sentence_history[game.cycle_state-1] == undefined || game.sentence_history[game.cycle_state-1].sentence == "none") {
...
function getType(nb_players_max) {
    ...
    if ((game.sentence_history[game.cycle_state] == undefined || game.sentence_history[game.cycle_state].sentence == "none") && game.cycle_state < game.sentence_amount) {
        posible_virus = true
    }

    var available_nature_probability = getNature();
    for (var i in available_nature_probability) {
    }
